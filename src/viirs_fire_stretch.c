	   ...
	   double *blue_cap, int *jpeg, char *outfile)
{
	...
	c = getopt_long(argc, argv, "hr:g:b:c:j?", long_options,
			&option_index);
		      ...
		      int jpeg)
{
	...
	papszOptions =
	    CSLSetNameValue(papszOptions, "PHOTOMETRIC", "YCBCR");
    } else {
	papszOptions =
	    CSLSetNameValue(papszOptions, "COMPRESS", "DEFLATE");
    }
...
char scale(double min, double max, double value)
{
    double result = 0.0;
...
int main(int argc, char *argv[])
{
    ...
    char *out_scan_line;
    char *out_scan_line_cleanup1, *out_scan_line_cleanup2,
	*out_scan_line_cleanup3;
	...
	for (y_index = 0; y_index < GDALGetRasterYSize(out_Dataset);
	     y_index++) {
	    /* Read data.. */
		...
		if (bands == 3) {
		    out_scan_line[x] =
			scale(0.0000001, blue_cap, data_scan_line[x]);
		} else {
		    out_scan_line[x] =
			scale(0.0000001, 1.0, data_scan_line[x]);
		};
	    }
	...
	/* Read data.. */
	GDALRasterIO(GDALGetRasterBand(out_Dataset, 1), GF_Read, 0,
		     y_index, xsize, 1, out_scan_line_cleanup1, xsize, 1,
		     GDT_Byte, 0, 0);
	GDALRasterIO(GDALGetRasterBand(out_Dataset, 2), GF_Read, 0,
		     y_index, xsize, 1, out_scan_line_cleanup2, xsize, 1,
		     GDT_Byte, 0, 0);
	GDALRasterIO(GDALGetRasterBand(out_Dataset, 3), GF_Read, 0,
		     y_index, xsize, 1, out_scan_line_cleanup3, xsize, 1,
		     GDT_Byte, 0, 0);
	for (x = 0; x < xsize; x++) {
	    if (out_scan_line_cleanup1[x] == 0
	...
	GDALRasterIO(GDALGetRasterBand(out_Dataset, 1), GF_Write, 0,
		     y_index, xsize, 1, out_scan_line_cleanup1, xsize, 1,
		     GDT_Byte, 0, 0);
	GDALRasterIO(GDALGetRasterBand(out_Dataset, 2), GF_Write, 0,
		     y_index, xsize, 1, out_scan_line_cleanup2, xsize, 1,
		     GDT_Byte, 0, 0);
	GDALRasterIO(GDALGetRasterBand(out_Dataset, 3), GF_Write, 0,
		     y_index, xsize, 1, out_scan_line_cleanup3, xsize, 1,
		     GDT_Byte, 0, 0);
